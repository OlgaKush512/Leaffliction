...
def validate_dataset_path(path):
    ...
    if not subdirs:
        raise ValueError(f"No subdirectories found in {path}. It should contain class subdirectories with images.")
    for subdir in subdirs:
        images = [f for f in os.listdir(os.path.join(path, subdir)) if f.lower().endswith(('jpg', 'jpeg', 'png'))]
        if not images:
