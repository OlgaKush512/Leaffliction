...
def apply_transformations(image_path, output_dir=None, save_all=True, save_original=False, save_gaussian=False, save_mask=False, save_roi=False, save_shape=False, save_pseudo=False, save_histogram=False):
    ...
    if save_pseudo or save_all:
        images_to_display.append(("Figure I.6: Pseudolandmarks", annotated_img))

    if output_dir is not None:
        plot_and_save_images(images_to_display, img, output_dir, filename, display=False, save_all=save_all, save_histogram = save_histogram)
    else:
        plot_and_save_images(images_to_display, img, display=True, save_all=save_all, save_histogram = save_histogram)

def plot_and_save_images(images_to_display, img, output_dir=None, filename=None, display=False, save_all=True, save_histogram = False):
    ...
    if output_dir and filename:
        ...
        plt.savefig(os.path.join(output_dir, f"{filename}_transformations.jpg"))
        plt.close()
    elif display:
        plt.show()


def process_directory(src_dir, dst_dir, save_all=True, save_original=False, save_gaussian=False, save_mask=False, save_roi=False, save_shape=False, save_pseudo=False, save_histogram=False):
    """Process all images in a directory."""
    for subdir, _, files in os.walk(src_dir):
        for file in files:
            if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                image_path = os.path.join(subdir, file)
                apply_transformations(image_path, dst_dir,save_all=save_all, 
...
def main():
    ...
    parser.add_argument('--original', action='store_true', help='Save the original image')
    parser.add_argument('--gaussian', action='store_true', help='Save the Gaussian blur image')
    parser.add_argument('--mask', action='store_true', help='Save the mask image')
    parser.add_argument('--roi', action='store_true', help='Save the ROI objects image')
    parser.add_argument('--shape', action='store_true', help='Save the shape analysis image')
    parser.add_argument('--pseudo', action='store_true', help='Save the pseudolandmarks image')
    parser.add_argument('--histogram', action='store_true', help='Save the color histogram image')

    args = parser.parse_args()

    save_all = not (args.original or args.gaussian or args.mask or args.roi or args.shape or args.pseudo or args.histogram)
