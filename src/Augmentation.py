...
def augment_image(image_path, output_dir, num_augmentations=None):
    ...
    augmentations = {
        ...
        "Rotate": cv2.warpAffine(
            ...
            cv2.getRotationMatrix2D(
                (image.shape[1] // 2, image.shape[0] // 2), 45, 1.0
            ),
        ...
        "Crop": image[
            int(image.shape[0] * 0.1): int(image.shape[0] * 0.9),
    ...
    for aug_type, aug_image in augmentations.items():
        ...
        aug_image_path = os.path.join(
            output_dir, f"{file_name}_{aug_type}{ext}"
        )
        ...
        for file in files:
            if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if img_hash not in image_hashes:
                    ...
                    print(f"Duplicate image found and skipped: {file_path} "
                          f"(duplicate of {image_hashes[img_hash]})")


def count_images_in_directories(src_dir):
    """Counts the number of img in each subdirectory of the source."""
    image_counts = {}
...
def balance_directories(dest_dir, max_images):
    """Balances the number of images in the new directory
    based on the maximum number from the original."""
    ...
    for subdir, _, files in os.walk(dest_dir):
        ...
        while current_count < max_images:
            for image in images:
                needed_augmentations = min(
                    to_augment,
                    max_images - current_count
                )
                added_augmentations = augment_image(
                    os.path.join(subdir, image),
                    subdir,
